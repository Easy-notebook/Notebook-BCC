...
class WorkflowCLI(ModernLogger):
        ...
        if 'notebook' in state_data:
            ...
            if notebook_id:
                ...
                if self.code_executor:
                    ...
                    self.info(f"[Sync] notebook_id: {notebook_id[:16]}...")
            else:
    ...
    def _build_idle_state(self, user_problem, user_submit_files, template_path=None):
        ...
        for candidate in template_candidates:
            if candidate.exists():
                ...
                # Generate new notebook_id for fresh start
                # (It will be synced to stores via _sync_state_to_stores)
                new_notebook_id = uuid.uuid4().hex
                idle_state['state']['notebook']['notebook_id'] = new_notebook_id
                print(f"  Generated notebook_id: {new_notebook_id[:16]}...")
    ...
    def _start_from_state_file(self, args):
        ...
        if notebook_data:
            ...
            notebook_id = notebook_data.get('notebook_id')
            if notebook_id:
                print(f"  Notebook: {cell_count} cells, ID: {notebook_id[:8]}...")
            else:
                print(f"  Notebook: {cell_count} cells (no notebook_id)")
